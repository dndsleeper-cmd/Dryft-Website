...
- **All pricing removed from the site + SEO** (per direction): deleted the `Offer`/`price`/`priceCurrency` node from the `SoftwareApplication` JSON-LD, removed the "How much is Dryft for students?" Q&A from both the visible FAQ and the `FAQPage` JSON-LD (now 4 questions, still in sync), and stripped all `$3.99` / `$8.99` / "two weeks free" / "verify your student status" copy. Verified zero pricing strings render on the page and the JSON-LD still parses. (The "CAD $100" liability cap in `terms.html` is legal language, not product pricing, and was left intact. `docs/archived-sections.html` still holds an old pricing block but is archived/not served.)
- **Free-offer framing removed**: the "Join free" nav CTA (index.html) and the "Join free" nav + mobile-menu CTAs (referral.html) are now "Join waitlist", matching the final-CTA button. Verified zero "free"/"trial" strings render. ("free-text" in privacy.html and "error-free" in terms.html are unrelated wording and were left.)
- **Branded search/app icon recolored to black** (the "blue one" Google shows): `assets/icons/icon.svg` master changed from teal `#0e4754` to black `#000000` (white D mark unchanged). Regenerated all rasters from it — `favicon.ico` (16/32/48/64 via Pillow), `favicon-96.png`, `apple-touch-icon.png`, `icon-192.png`, `icon-512.png` (rendered from the SVG via headless Chrome → `sips`). Bumped icon version tokens to `?v=20260604b` across `index.html` + `privacy/terms/referral.html` (required because `/assets/*` is now immutable). The adaptive light/dark tab favicon (`assets/favicon.svg`) was already black/white and left unchanged.
